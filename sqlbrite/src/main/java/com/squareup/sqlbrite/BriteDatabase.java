...
public final class BriteDatabase implements Closeable {
  ...
  private final Consumer<Subscription> ensureNotInTransaction = new Consumer<Subscription>() {
    @Override public void accept(@io.reactivex.annotations.NonNull
  ...
  BriteDatabase(SQLiteOpenHelper helper, SqlBrite.Logger logger, Scheduler scheduler,
                FlowableTransformer<Query, Query> queryTransformer) {
    this.helper = helper;
    this.logger = logger;
  ...
  public QueryObservable createQuery(@NonNull final String table, @NonNull String sql,
      @NonNull String... args) {
    Predicate<Set<String>> predicate = new Predicate<Set<String>>() {
      @Override public boolean test(@io.reactivex.annotations.NonNull Set<String> triggers) throws Exception {
        return triggers.contains(table);
      }
    };
    return createQuery(predicate, sql, args);
  }
  ...
  private QueryObservable createQuery(Predicate<Set<String>> tableFilter, String sql,
    ...
    final Flowable<Query> queryObservable = triggers //
    ...
    return new QueryObservable(new FlowableOnSubscribe<Query>() {
      @Override public void subscribe(@io.reactivex.annotations.NonNull FlowableEmitter<Query> e) throws Exception {
        e.setDisposable(queryObservable.subscribe());
      }
    });
  }
  ...
  final class DatabaseQuery extends Query implements Function<Set<String>, Query> {
    private final Predicate<Set<String>> tableFilter;
    private final String sql;
    private final String[] args;

    DatabaseQuery(Predicate<Set<String>> tableFilter, String sql, String... args) {
     ...
     *
     * @param strings the input value
     * @return the output value
     * @throws Exception on error
     */
    @Override public Query apply(@io.reactivex.annotations.NonNull Set<String> strings) throws Exception {
      return this;
    }
  }
}
